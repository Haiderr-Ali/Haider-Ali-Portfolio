...
const Timeline: React.FC<TimelineProps> = ({ experiences }) => {
  return (
    <div className="relative max-w-screen-lg mx-auto px-4 sm:px-6">
      {/* Vertical line */}
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-0.5 h-full bg-gray-700 sm:block hidden"></div>
      ...
      <div className="space-y-12">
        {experiences.map((experience, index) => (
          <motion.div
            ...
            } sm:space-x-8`} // Adds space between items only on larger screens
            initial={{ opacity: 0, x: index % 2 === 0 ? 20 : -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8, delay: index * 0.2 }}
